...
Fill the setings as I have above or tweak them to your needs. Make sure to copy the public key as you'll need that for the configuration file being built in the next step.

Got to Interfaces -> Assignments and there should be a new unassigned interface. Give it a description such as mgmt_vpn and click Add. Then go to the interface and enable it.
...
Set the keepalive timeout to 25 or else the connection will not work.
...
Set the Source to Single Host or Netowrk with a value of 192.168.122.1. This effectively ensures that only local connections will be allowed as that is the IP address of the WAN bridge interface.
...
The first rule that will be created is to support DNS. Create a rule with the ad_lan as the interface, change the source to "ad_lan net" so that all of the machines on the internal AD domain will have DNS support. Set the prtotocol to UDP and the destination port to "Single port or range" with a value of 53.

The next rule will support Network Time Protocol(NTP) for the DC. All of the machines in the domain will use the DC as an NTP server so only the DC needs NTP traffic. The rule will be similar to the previous one but the source will be the DC IP and the destination port will be 123.

Next, two ad_lan-wide rules will be created. one for port 80 and one for port 443. This way, all of the AD joined machines will be able to reach out to the internet which is realistic for an enterprise environment. You should now be able to run updates on your AD's DC.