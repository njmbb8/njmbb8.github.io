# Advent of Cyber 2025 SideQuest 0: Hopper's Origins Writeup
...
![Well that looks interesting, i wonder what that does!](vanchat_prompt.png)
...
![Shell POPPED! WOOT!](WEB_shell_n_flag.png)
...
![enter image description here](DB_initial_access.png)
...
![enter image description here](manual_routing.png)
...
![enter image description here](DB_nmap.png)
...
Given that we don't have any credentials, the web server on .101 should be our first target. LDAP on .122 is also interesting from an information gathering standpoint; however, enum4linux is unable to gather any information, so we'll put that on the back burner for when we have domain credentials. 

Visiting the web server brings us to the VanChat Printer Hub which gives us a form to test ldap authentication against the DC at .122. There is a username already populated, "anne.clark@vanchat.loc" and a prepopulated password field. Inspecting the page source shows that the password in the password field is not real so no joy there. We are, however, able to control the host and port that the authentication request is sent to which means that if we can send it to a responder listener on our attacker machine, we can get creds! 
...
![enter image description here](anne_clark_creds.png)
...
![enter image description here](as-rep_roastable_ai.png)
...
![enter image description here](server1_rdp.png)

## SERVER1 Privilege Escalation
...
![enter image description here](SERVER1_pwn.png)
...
![enter image description here](server1_mimikatz.png)
...
![enter image description here](server1_sharpdpapi.png)
...
![enter image description here](server2_user.png)
...
![enter image description here](SERVER2_root.png)
...
![enter image description here](SERVER2_generic_all.png)
...
## Pwning the ai.vanchat.loc
In the RDP session as the admin user, we can use PsExec again to open a SYSTEM shell with `PsExec64.exe -s -i cmd.exe` and then run `net user administrator P455w0rd /domain` to update the password of the domain administrator to P455w0rd. We can then use RDP to access DC1.ai.vanchat.loc and grab the user and admin flags.

![enter image description here](ai_vanchat_pwnd.png)

## Moving to, and then pwning, vanchat.loc
...
![enter image description here](vanchat_loc_pwnd.png)
...
![enter image description here](SERVER3_pwn.png)
...
![enter image description here](SQL_Protection_Policy.png)

Let's update that to now allow RDP access to all domain users.

![enter image description here](SQLPP_updated.png)
...
## Pivoting off of SERVER3
![enter image description here](SERVER3_SPN.png)
...
![enter image description here](SERVER4_user.png)
...
## Accessing the TBFC.loc domain from the attacker machine
Adding a route for our new machine(SERVER4, .141) through the interface we set up for traffic through SERVER3, we can nmap the host and find rdp and winrm ports open in addition to mssql on 1433. Let's take advantage of our admin foothold and create a new local admin to RDP into the machine with.
...
![enter image description here](jack_permissions.png)
...
![enter image description here](server_perms.png)
...
![enter image description here](bloodhound_adcs.png)

At this point, we need to start touching the TBFC DC, so we set up another agent on SERVER4. With the tunnel created, we should have everything that we need to perform our attack from our kali machine except for a way to authenticate as the machine account. We can grab this from mimikatz and from there run `certipy find -u 'tbfc-sqlserver1$@tbfc.loc' -hashes :177d7ca2f68c9abd23815096c9aae9ae -dc-ip 10.200.171.131 -vulnerable -stdout` to gather the rest of the information that we need to get a certificate to authenticate as domain admin: `certipy req -u 'TBFC-SQLSERVER1$@tbfc.loc' -hashes :177d7ca2f68c9abd23815096c9aae9ae -template TBFCWebServer -upn Administrator@tbfc.loc -dc-ip 10.200.171.131 -dc-host TBFC-DC1.tbfc.loc -ca TBFC-CA -out da.pfx`. Once we have the certificate, we can use it to get a kerberos ticket as the domain admin as well as the admin's hash using `certipy auth -pfx da.pfx -dc-ip 10.200.171.131`. We can then take the hash and use it in `evil-winrm -i 10.200.171.131 -u administrator -H bc42803c87460f4a2bce81f190209b15` to get the final flags!

![enter image description here](challenge_pwn.png)
