...
In this step, we'll be adding a creating a Kali Linux VM to attack our AD network. The idea is to simulate an attacker from an untrusted external network. A VPN can be added later so that an attack can be launched from the perspective of an assumed breach or just so that the dmz would not need to be reexploited every time I want to test something in the internal network.
...
As I'm planning on using this for other challenges, I gave my system significantly more than the recommended minimum requirements: 4 cpus, 8GB of memory, and 120 GB of storage.
...
Once you reboot the computer, go to View -> Scale Display -> Always and check he box for "Auto Resize VM with Window". You should now be able to share your clipboard with the VM and the screen should adjust to the window resizing.
